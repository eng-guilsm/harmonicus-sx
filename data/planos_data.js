// Gerado automaticamente pelo MoneyLab Data Engine (2026-08-22 15:47:50)
window.PLANOS_TACTICAL_DATA = [
  {
    "id": 1,
    "nome": "Plano Guiana Brasileira",
    "badge": "🥇 Macro Rotação",
    "categoria": "baixo_risco",
    "par": "PAXG <-> BTC",
    "lote_brl": 100.0,
    "lucro_min_pct": 1.5,
    "gatilho_desc": "Z <= -1.65σ (Topo BTC -> Ouro) / Z >= +1.65σ (Fundo BTC -> BTC)",
    "trava_ruptura": "Modulação CWT Morlet (Ew < 50: Lote 100% | Ew >= 50: Lote 50%)",
    "cooldown_horas": 4.0,
    "valor_atual_str": "Ratio 0.05941 (Z: -1.04σ)",
    "alvo_str": "Ratio <= 0.05580 (Alvo BTC: ~R$ 424.000)",
    "distancia_display": "+6.5% de alta no BTC",
    "proximidade_score": 48,
    "status": "SENTINELA_QUENTE",
    "icone": "🥇",
    "cor": "#F59E0B"
  },
  {
    "id": 2,
    "nome": "Plano Escudo de Aquiles",
    "badge": "🛡️ VIX Dip-Hunter",
    "categoria": "baixo_risco",
    "par": "BRL -> BTC",
    "lote_brl": 200.0,
    "lucro_min_pct": 2.0,
    "gatilho_desc": "VIX >= 22.50 E (PC1 >= 40% ou S <= 1.75)",
    "trava_ruptura": "Anti-Faca Caindo: Exige desaceleração de onda Ew < 50.0",
    "cooldown_horas": 4.0,
    "valor_atual_str": "VIX = 16.09 pts",
    "alvo_str": "VIX >= 22.50 pts (Pânico Global)",
    "distancia_display": "+6.41 pts de estresse no VIX",
    "proximidade_score": 71,
    "status": "AGUARDANDO_PANICO",
    "icone": "🛡️",
    "cor": "#10B981"
  },
  {
    "id": 3,
    "nome": "Plano Pátria Volátil",
    "badge": "💵 Dollar-Peg PTAX",
    "categoria": "baixo_risco",
    "par": "BRL <-> USDT",
    "lote_brl": 200.0,
    "lucro_min_pct": 0.8,
    "gatilho_desc": "Desconto USDT >= R$ 0.0400 vs PTAX Comercial",
    "trava_ruptura": "Ancorado no ágio estrutural histórico de 79.6% do tempo",
    "cooldown_horas": 4.0,
    "valor_atual_str": "USDT R$ 5.1625 (Spread: +0.1510)",
    "alvo_str": "Spread <= -R$ 0.0400 vs PTAX",
    "distancia_display": "Faltam R$ 0.1910 de desconto",
    "proximidade_score": 35,
    "status": "AGUARDANDO_DESCONTO",
    "icone": "💵",
    "cor": "#3B82F6"
  },
  {
    "id": 4,
    "nome": "Plano Caboclo dos Oráculos",
    "badge": "🌐 Infraestrutura LINK",
    "categoria": "baixo_risco",
    "par": "BRL <-> LINK",
    "lote_brl": 100.0,
    "lucro_min_pct": 2.2,
    "gatilho_desc": "Reversão à Média 51h (Z_LINK <= -2.00σ)",
    "trava_ruptura": "Trava Causal: Exige STE >= 0 e Ew < 50 (Caudas de Lévy)",
    "cooldown_horas": 4.0,
    "valor_atual_str": "LINK R$ 60.44 (Z: +6.39σ)",
    "alvo_str": "Preço <= R$ 47.30 (Z <= -2.0σ)",
    "distancia_display": "+27.8% de recuo",
    "proximidade_score": 25,
    "status": "MONITORANDO_BANDA",
    "icone": "🌐",
    "cor": "#8B5CF6"
  },
  {
    "id": 5,
    "nome": "Plano Gravidade Zero",
    "badge": "🪐 Spillover Macro SOL",
    "categoria": "baixo_risco",
    "par": "BTC <-> SOL",
    "lote_brl": 50.0,
    "lucro_min_pct": 3.0,
    "gatilho_desc": "PC1 >= 40% com Lag de Autovetor |v_SOL|",
    "trava_ruptura": "Trava Pânico: Bloqueado se PC1 > 70% (Colapso Sistêmico)",
    "cooldown_horas": 4.0,
    "valor_atual_str": "SOL/BTC = 0.00122",
    "alvo_str": "Ratio <= 0.00207",
    "distancia_display": "-41.0% de descolamento",
    "proximidade_score": 20,
    "status": "SENTINELA_CICLICA",
    "icone": "🪐",
    "cor": "#EC4899"
  },
  {
    "id": 6,
    "nome": "Plano Corisco da Solana",
    "badge": "⚡ SOL Bounce 15m",
    "categoria": "medio_risco",
    "par": "BRL <-> SOL",
    "lote_brl": 50.0,
    "lucro_min_pct": 0.9,
    "gatilho_desc": "Banda Inferior Bollinger 15m (Z <= -2.0σ)",
    "trava_ruptura": "Filtro Causal: Exige STE >= 0, PC1 < 70% e Ew < 50",
    "cooldown_horas": 1.5,
    "valor_atual_str": "SOL R$ 486.80 (Z_15m: +5.30σ)",
    "alvo_str": "Preço <= R$ 420.30",
    "distancia_display": "+15.8% para banda inferior",
    "proximidade_score": 60,
    "status": "AGUARDANDO_CORRECAO_15M",
    "icone": "⚡",
    "cor": "#F59E0B"
  },
  {
    "id": 7,
    "nome": "Plano Duelo de Titãs",
    "badge": "⚔️ Micro-Pairs ETH/BTC",
    "categoria": "medio_risco",
    "par": "BTC <-> ETH",
    "lote_brl": 60.0,
    "lucro_min_pct": 1.0,
    "gatilho_desc": "Desvio do Ratio Diário 24h (|Z_ETH/BTC| >= 1.50σ)",
    "trava_ruptura": "Coerência de Fase γ_ETH/BTC = 71.0% | Bloqueio PC1 > 75%",
    "cooldown_horas": 1.5,
    "valor_atual_str": "ETH/BTC = 0.03142 (Z: +1.16σ)",
    "alvo_str": "|Z| >= 1.50σ (Gatilho de Arbitragem)",
    "distancia_display": "Faltam 0.34σ para disparo",
    "proximidade_score": 82,
    "status": "ZONA_DE_DISPARO_IMINENTE",
    "icone": "⚔️",
    "cor": "#06B6D4"
  },
  {
    "id": 8,
    "nome": "Plano Flecha de Sagarana",
    "badge": "🏹 BTC Micro-Dip 5m",
    "categoria": "medio_risco",
    "par": "BRL -> BTC",
    "lote_brl": 75.0,
    "lucro_min_pct": 0.8,
    "gatilho_desc": "Micro-Queda 5m <= -0.40% (> 1.5σ intradiário)",
    "trava_ruptura": "Trava Espectral: Exige Ew < 50.0 (Sem tempestade de choque)",
    "cooldown_horas": 1.5,
    "valor_atual_str": "Retorno 5m = +10.91%",
    "alvo_str": "Retorno 5m <= -0.40%",
    "distancia_display": "Faltam +11.31% de recuo rápido",
    "proximidade_score": 75,
    "status": "RADAR_ATIVO_5M",
    "icone": "🏹",
    "cor": "#EAB308"
  }
];

window.PORTFOLIO_STATE = {
  "total_brl": 1709.72,
  "aportado_brl": 1731.0,
  "prejuizo_residual_brl": -21.28,
  "caixa_brl": 1150.0,
  "caixa_pct": 67.26,
  "btc_brl": 448.19,
  "btc_qtd": 0.00112671,
  "btc_pct": 26.21,
  "paxg_brl": 108.04,
  "paxg_qtd": 0.00456335,
  "paxg_pct": 6.32,
  "usdt_brl": 3.49,
  "usdt_qtd": 0.6764,
  "usdt_pct": 0.21,
  "cotacoes_ao_vivo": {
    "BTCBRL": 398647.0,
    "USDTBRL": 5.1625,
    "PAXGBRL": 23685.446749999996,
    "SOLBRL": 486.8,
    "ETHBRL": 12527.03,
    "LINKBRL": 60.44,
    "BNBBRL": 3602.0,
    "ADABRL": 1.179,
    "VIX": 16.09,
    "USDBRL_COMERCIAL": 5.011499881744385
  },
  "harmonicus_sensores": {
    "pc1": 0.6284,
    "entropia": 1.2425,
    "energia_fourier": 20.9885,
    "energia_wavelet_morlet": -0.5913,
    "fluxo_ste": 0.0,
    "regime": "RESONANCIA_SISTEMICA",
    "fator_lote": 1.0
  }
};
